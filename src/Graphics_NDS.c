...
// VRAM banks configuration
// Bank A: textures, B: texture, C: texture, D: texture, E: palettes

// Fixed-point macros (20.12)
...
static int lastMatrix = -1;

// Vertex format definitions
enum VertexFormat_ { VERTEX_FORMAT_TEXTURED, VERTEX_FORMAT_COLOURED };
...
// Queue a textured vertexoid Graphics_QueueVertex(int x, int y, int z, int u, int v, u16 c) {
    if (dsVertCount >= MAX_VERTS) return;
    dsVertices[dsVertCount++] = (VertexTextured){FP(x), FP(y), FP(z), FP(u), FP(v), c};
}

// Flush DS bufferoid Graphics_EndFrameDS() {
    if (!dsVertCount) return;
    glBegin(GL_QUADS);
    for (int i = 0; i < dsVertCount; i++) {
        VertexTextured* v = &dsVertices[i];
        glColor3b((v->c>>11)&0x1F, (v->c>>6)&0x1F, (v->c>>1)&0x1F);
...
// Full original engine code below
